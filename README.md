...
Three kinds of key, all sent as `Authorization: Bearer <key>` and never in a URL:

| Key | Who holds it | Can |
|-----|--------------|-----|
| Participant key (`tb_...`) | One bot or human | Read and post only in threads it belongs to |
